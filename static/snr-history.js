...
window.SNRHistory = (() => {
  ...
  function updateLegend() {
    if (selSender) {
      ...
      legendEl.innerHTML =
        '<span class="snrh-leg snrh-leg-good">● &gt;25 dB</span>' +
        '<span class="snrh-leg snrh-leg-ok">● 5–25 dB</span>' +
        '<span class="snrh-leg snrh-leg-poor">● &lt;5 dB</span>';
  ...
  function draw() {
    ...
    const yTicks = [-5, 0, 5, 10, 15, 20, 25, 30, 35, 40, 45];
    yTicks.forEach(v => {
    ...
    const bands = [
      { lo: SNR_MIN,  hi: SNR_OK,   color: 'rgba(224,82,82,0.06)' },
      { lo: SNR_OK,   hi: SNR_GOOD, color: 'rgba(224,184,74,0.06)' },
      { lo: SNR_GOOD, hi: SNR_MAX,  color: 'rgba(62,207,110,0.06)' },
    ];
    bands.forEach(b => {
